...
int main()
{
    ...
    /// ########################################### Chargement des données à partir du flot (décommenter pour le fichier voulu)
...
//    lecteur.sauvegarderDonnees("save.txt",complexe2);
...
////    complexe7.ComplexeCubique<3,int,5>::getIterator()->getCellI(0)->setListePointICellule(p2);
    ...
    /// ########################################### Chargement des données à la main et tests des différentes méthodes (tout décommenter une seule fois)
    ...
    cout << endl << "Simplification du complexe : " << endl;
    complexe.ComplexeCubique<3,int,2>::simplificationComplexe();

    cout << endl << "Validité du complexe après simplification : " << endl;
    if (complexe.estValide())cout << "Valide ! " << endl;
    else cout << "Non valide !" << endl;
    cout << endl;
