...
%token TOK_IDENT // [a-zA-Z]([a-zA-Z0-9])+	
%token TOK_INTEGER
%token TOK_REAL
%token TOK_BOOLEAN
%token TOK_STRING
...
BlockDeclVar			:	KW_VAR ListDeclVar SEP_SCOL
			 	|
			 	;

ListDeclVar			:	ListDeclVar SEP_SCOL DeclVar
			 	|	DeclVar
			 	;

DeclVar				:	ListIdent SEP_DOTS Type
			 	;
...
ArrayType			:	KW_ARRAY SEP_CO InterBase SEP_DOTDOT InterBase SEP_CF KW_OF BaseType
			 	;

InterBase			:	TOK_INTEGER
			 	|	OP_SUB TOK_INTEGER
			 	;
